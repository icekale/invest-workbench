<template>
  <nav class="guanlan-tabbar" aria-label="主导航">
    <router-link
      v-for="item in tabs"
      :key="item.path"
      :to="item.path"
      class="guanlan-tabbar-item"
      :class="{ 'is-active': route.path.startsWith(item.match) }"
    >
      <component :is="item.icon" class="guanlan-tabbar-icon" />
